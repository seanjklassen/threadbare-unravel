...
void UnravelProcessor::processBlock(juce::AudioBuffer<float>& buffer, juce::MidiBuffer& midi)
{
    ...
    currentState.size = clamp(readParam(sizeParam, 1.0f), 0.5f, 2.0f);
    currentState.decaySeconds = clamp(readParam(decayParam, 5.0f), 0.4f, 20.0f);
...
juce::AudioProcessorValueTreeState::ParameterLayout UnravelProcessor::createParameterLayout()
{
    ...
    auto decayRange = juce::NormalisableRange<float>(0.4f, 20.0f);
